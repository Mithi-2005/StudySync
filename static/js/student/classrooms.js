...
// Simple animation on scroll
  ...
  function loadCourses() {
    ...
    fetch(`/administrator/get-courses-json?department=${department}&section=${section}&semester=${semester}`)
        .then(response => response.json())
        .then(data => {
            CoDropdown.innerHTML = '<option value="" selected disabled>Select course</option>' +
  ...
  function gotoClass(id) {
    window.open(`/classroom/${id}/`, '_self');
  }
  ...
  function joinClass(event) {
    ...
    const classCode = document.getElementById("edit_class_name").value.trim();
    console.log(classCode);
    const csrfToken = document.querySelector("[name=csrfmiddlewaretoken]").value;
  
    if (classCode.length !== 6) {
      showToast("Invalid class code. It must be 6 characters.", "error");
      return;
    }
    ...
    fetch("/classroom/join/", {
      ...
      .then((data) => {
        ...
        if (data.status === "success") {
          ...
          closeEditPopup();
          setTimeout(() => {
            location.reload(); // Refresh to update class list
          }, 1000);
        } else {
